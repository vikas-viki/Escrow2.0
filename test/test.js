...
describe('Escrow', function () {
  ...
  describe('New List', () => {
    ...
    it('if not approved send back the amount deposited by buyer', async ()=>{
      const depositorBalanceBefore = await ethers.provider.getBalance(depositor.address);
      const escrowContract = new ethers.Contract(Escrows.listings(0), Escrow.interface, beneficiary);
      const tx = await escrowContract.connect(arbiter).approve(false);
      await tx.wait();
      expect(await escrowContract.isApproved()).to.equal(false);
      // Check if the buyer balance updates after the buyer buys the product (approved).
